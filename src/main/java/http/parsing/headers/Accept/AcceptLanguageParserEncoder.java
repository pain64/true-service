...
public class AcceptLanguageParserEncoder implements ListHeaderParser<LanguageRangeWithWeight, AcceptLanguage> {
    public static LanguageRange LANGUAGE_RANGE(RequestByteStream bs, Buffer bfr) {
        if (IS_CHAR(bs, '*')) {bs.advance(); return new LanguageRange.Star();}

        TOKEN_ALPHA(bs, bfr);

        if (bfr.remains() > 8)
            throw new DecodeException(bs, "Expected 1*8ALPHA, has " + bfr.remains() + " ALPHA");
        ...
        if (bfr.remains() > 8)
            throw new DecodeException(bs, "Expected 1*8alphanum, has " + bfr.remains() + " alphanum");
    ...
    @Override
    public void decode(RequestByteStream bs, Buffer bfr, ArrayList<LanguageRangeWithWeight> dest) {
