...
public class JumpTables {
    ...
    public static final boolean[] IS_COOKIE_TABLE = new boolean[TABLE_SIZE];
    static
    {
        for (var i = 0; i < TABLE_SIZE; i++) {
            var b = (byte) i;
            IS_COOKIE_TABLE[i] = (b == '!') || (b >= '#' && b <= '+') || (b >= '-' && b <= ':') || (b >= '<' && b <= '[') || (b >= ']' && b <= '~');
        }
    }
    ...
    public static final boolean[] IS_PERCENT_ENCODING_TABLE = new boolean[TABLE_SIZE];
    static
    {
        //":" / "/" / "?" / "#" / "[" / "]" / "@"
        //"!" / "$" / "&" / "'" / "(" / ")" / "*" / "+" / "," / ";" / "="
